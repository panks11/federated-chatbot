...
# model = m.LSTM(len(features.tokenizer.index_word), len(features.label_map))
model = m.TextClassificationModel(
    len(features.tokenizer.index_word), 64, len(features.label_map) + 1
)
accuracy_list, loss_list, val_accuracy_list, val_loss_list = i_train.train(
    model, train_dl, val_dl, epochs=40
)
