# experiments/run_federated_ann.py
...
def main():
    ...
    logger = get_logger("fed_ann", base_config["logging"]["log_dir"])
    ...
    for rnd in range(base_config["federated"]["rounds"]):
        ...
        for i, (_, _, test_loader, _) in enumerate(client_models):
            ...
            total_acc += acc
            logger.info(f"Client {i} Accuracy: {acc:.4f}")
        logger.info(f"Average Accuracy After Round {rnd+1}: {total_acc / len(client_models):.4f}")
